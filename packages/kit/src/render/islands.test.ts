...
describe('renderIslands', () => {
  it('SSR-renders static content and island subtrees', () => {
  ...
  it('marks each client mode and x-ignores every island', () => {
  ...
  it('reports zero hydrating islands for a fully static page', () => {
    const { hydratingCount } = renderIslands('<p>hello</p>', {}, scope)
    expect(hydratingCount).toBe(0)
  })

  it('counts a none-only page as zero hydrating (ships no JS)', () => {
    const { hydratingCount } = renderIslands('<div x-data="{}" client:none></div>', {}, scope)
    expect(hydratingCount).toBe(0)
  })
})
