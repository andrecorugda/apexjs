...
var m=String(n).indexOf('node:')===0?String(n).slice(5):String(n);
...
if(m==='url')return {pathToFileURL:function(p){return {href:'file://'+p};},fileURLToPath:function(u){u=String(u);return u.indexOf('file://')===0?u.slice(7):u;},URL:globalThis.URL};
if(m==='http')return {createServer:function(){throw new Error('no http server on device');},STATUS_CODES:{},METHODS:['GET','POST','PUT','PATCH','DELETE','HEAD','OPTIONS']};
if(m==='crypto')return {randomBytes:function(len){var a=new Uint8Array(len);if(globalThis.crypto&&globalThis.crypto.getRandomValues)globalThis.crypto.getRandomValues(a);else for(var i=0;i<len;i++)a[i]=Math.floor(Math.random()*256);return a;},randomUUID:function(){var b=this.randomBytes(16);b[6]=(b[6]&15)|64;b[8]=(b[8]&63)|128;var h='';for(var i=0;i<16;i++)h+=(b[i]+256).toString(16).slice(1);return h.slice(0,8)+'-'+h.slice(8,12)+'-'+h.slice(12,16)+'-'+h.slice(16,20)+'-'+h.slice(20);},getRandomValues:function(a){if(globalThis.crypto&&globalThis.crypto.getRandomValues)globalThis.crypto.getRandomValues(a);return a;},webcrypto:globalThis.crypto,timingSafeEqual:function(a,b){if(!a||!b||a.length!==b.length)return false;var r=0;for(var i=0;i<a.length;i++)r|=a[i]^b[i];return r===0;},createHash:function(){throw new Error('crypto.createHash is unavailable on the bare mobile engine — hash server-side or use the pure-JS path');},createHmac:function(){throw new Error('crypto.createHmac is unavailable on the bare mobile engine');}};
...
export const access=nope,appendFile=nope,chmod=nope,chown=nope,copyFile=nope,cp=nope,lchmod=nope,lchown=nope,link=nope,lstat=nope,lutimes=nope,mkdir=nope,mkdtemp=nope,open=nope,opendir=nope,readFile=nope,readdir=nope,readlink=nope,realpath=nope,rename=nope,rm=nope,rmdir=nope,stat=nope,statfs=nope,symlink=nope,truncate=nope,unlink=nope,utimes=nope,watch=nope,writeFile=nope
export default {access,appendFile,chmod,chown,copyFile,cp,lchmod,lchown,link,lstat,lutimes,mkdir,mkdtemp,open,opendir,readFile,readdir,readlink,realpath,rename,rm,rmdir,stat,statfs,symlink,truncate,unlink,utimes,watch,writeFile}
`
