...
- 7d4ac0d: feat(mobile): `apex mobile doctor` — check the native toolchain and help set it up
  ...
  - `--fix` runs the _safe_ automatable steps with consent — accept SDK licenses, `sdkmanager`
...
- 0239e42: fix(mobile): run gradle.bat / gradlew.bat on Windows (Node EINVAL on batch files)
...
- 0941cde: feat(mobile): make `apex mobile android --assemble` build the APK with a standalone Gradle
...
- 5ce26db: fix(mobile): clean, cross-platform handling when the native toolchain is absent
...
- fda6130: Fix the dev server opening a second port for Vite HMR (`port + 1`, e.g. 3001). On Windows that
...
- fb26ae3: Fix `apex upgrade` not actually updating `@apex-stack/*` on existing apps. It ran a plain
...
- Updated dependencies [7c3bef6]
  - @apex-stack/kit@0.10.1
  - @apex-stack/vite@0.5.1
...
- Updated dependencies [a39517f]
- Updated dependencies [613fd3c]
  - @apex-stack/vite@0.5.0
  - @apex-stack/kit@0.10.0
...
- 1250ffe: Platform pillars (1.1): four new subsystems exported from `@apex-stack/core/server`.
...
- f5b1d29: SSR page responses now send `Cache-Control: no-store`. Server-rendered HTML embeds per-request /
...
- 9821fb8: Fix `apex build` / `--islands` for i18n apps (#54). The static prerender loop never built the
