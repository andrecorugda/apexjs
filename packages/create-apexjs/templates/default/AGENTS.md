...
apex extend pwa                   # add a pwa config block + icons → installable + offline (manifest + service worker at build)
...
apex mobile android --assemble --out MyApp.apk  # package the whole app as an offline APK (on-device SSR+API), copied to ./MyApp.apk
apex test                         # run Vitest
```
**Ship it as a native app.** `apex build --mobile` bundles the *entire* app (SSR + API + on-device SQLite + auth) into a self-contained JS bundle that runs on a bare on-device engine — offline, no server. `apex mobile android --assemble` scaffolds a WebView shell around it and builds an installable APK. That needs a JDK 17+, the Android SDK, and Gradle — **not** Android Studio. Run **`apex mobile doctor`** to check the toolchain and get install links + exact setup commands (or `apex mobile doctor --fix` to auto-install the safe bits: SDK packages, licenses, `local.properties`). If Gradle isn't on `PATH`, point at it and your SDK: `apex mobile android --assemble --gradle <…/bin/gradle[.bat]> --sdk <android-sdk-dir>` (add `--wrapper` once to generate `gradlew` so later builds need only a JDK). `--out MyApp.apk` copies the assembled APK to a named file instead of leaving it in Gradle's build outputs. iOS: `apex mobile ios` scaffolds a WKWebView shell anywhere, but building/signing an IPA needs a Mac + Xcode.
...
server/hooks.ts     Optional. defineHooks({ onRequest, onError, onShutdown }) — request log
                     override, error reporting (Sentry-style), shutdown cleanup. Prod-loaded.
...
- **Route** → `defineApexRoute({ method, input: { …zod }, mcp: true, auth: true, can, handler })`. `mcp: true` makes it an AI-callable tool at `/mcp`. Write routes honor an `Idempotency-Key` request header out of the box — a retried POST with the same key replays the cached response instead of re-executing (safe client retries).
...
## Deploy
`apex build --preset vercel|netlify|docker`. Set `DATABASE_URL` (Supabase/Neon/Postgres or Turso) + `APEX_SESSION_PASSWORD` (≥32 chars) in the host env. In production `apex start` serves `/health` + `/healthz` (liveness), logs one JSON line per request (`--quiet` / `APEX_LOG=off` to silence), drains in-flight requests on SIGTERM/SIGINT, and closes DB handles automatically — `server/hooks.ts` customizes all of it. Error bodies are masked in prod (generic 500; the detail goes to logs / `onError`). See the deploy docs.
